...
struct GlobalAttrInfo : public ParsedAttrInfo {
	GlobalAttrInfo() {
		/**
		...
		OptArgs = 15;
		/**
		 * GNU-style __attribute__(("global")) and C++/C23-style [[global]]
	...
	bool diagAppertainsToDecl(Sema &S, const ParsedAttr &Attr,
		...
		if (!isa<FunctionDecl>(D) && !isa<VarDecl>(D)) {
			unsigned ID = S.getDiagnostics().getCustomDiagID(DiagnosticsEngine::Error,
						"'" GLOBAL_ATTR_NAME "' attribute only allowed at function and variable");
	...
	AttrHandling handleDeclAttribute(Sema &S, Decl *D,
		...
		if (!D->getDeclContext()->isFileContext()) {
			unsigned ID = S.getDiagnostics().getCustomDiagID(DiagnosticsEngine::Error,
						"'" GLOBAL_ATTR_NAME "' attribute only allowed at file scope");
			S.Diag(Attr.getLoc(), ID);
		...
		if (Attr.getNumArgs() > 0) {
			unsigned ID = S.getDiagnostics().getCustomDiagID(DiagnosticsEngine::Error,
						"'" GLOBAL_ATTR_NAME "' attribute no need arguments");
			S.Diag(Attr.getLoc(), ID);
			return AttributeNotApplied;
		}
	...
	AttrHandling handleStmtAttribute(Sema &S, Stmt *St, const ParsedAttr &Attr,
		...
		if (Attr.getNumArgs() > 0) {
			...
			Result = AnnotateAttr::Create(S.Context, GLOBAL_ATTR_NAME, nullptr, 0,
						Attr.getRange());
		}
		return AttributeApplied;
	}
...
static ParsedAttrInfoRegistry::Add<fakecuda::frontend::attribute::GlobalAttrInfo> X(GLOBAL_ATTR_NAME, "");
