...
obj_func_sh_addr=$(sec2sh_addr ${OBJ} ${obj_func_sec})
obj_func_sh_offset=$(sec2sh_offset ${OBJ} ${obj_func_sec})
obj_func_st_value=$(sym2value ${OBJ} ${FUNC})

exe_func_sec=$(sym2sec ${EXE} ${FUNC})
exe_func_sh_addr=$(sec2sh_addr ${EXE} ${exe_func_sec})
exe_func_sh_offset=$(sec2sh_offset ${EXE} ${exe_func_sec})
exe_func_st_value=$(sym2value ${EXE} ${FUNC})

obj_sym_st_value=$(sym2value ${OBJ} ${SYM})
obj_sym_sec=$(sym2sec ${OBJ} ${SYM})
obj_sym_sh_addr=$(sec2sh_addr ${OBJ} ${obj_sym_sec})
obj_sym_sh_offset=$(sec2sh_offset ${OBJ} ${obj_sym_sec})

exe_sym_st_value=$(sym2value ${EXE} ${SYM})
exe_sym_sec=$(sym2sec ${EXE} ${SYM})
exe_sym_sh_addr=$(sec2sh_addr ${EXE} ${exe_sym_sec})
exe_sym_sh_offset=$(sec2sh_offset ${EXE} ${exe_sym_sec})

printf "obj: %s : section %-2d, sh_addr %s, sh_offset %s, st_value %s\n" \
	${FUNC} ${obj_func_sec} ${obj_func_sh_addr} ${obj_func_sh_offset} ${obj_func_st_value}
printf "exe: %s : section %-2d, sh_addr %s, sh_offset %s, st_value %s\n" \
	${FUNC} ${exe_func_sec} ${exe_func_sh_addr} ${exe_func_sh_offset} ${exe_func_st_value}

printf "obj: %s : section %-2d, sh_addr %s, sh_offset %s, st_value %s\n" \
	${SYM} ${obj_sym_sec} ${obj_sym_sh_addr} ${obj_sym_sh_offset} ${obj_sym_st_value}
printf "exe: %s : section %-2d, sh_addr %s, sh_offset %s, st_value %s\n" \
	${SYM} ${exe_sym_sec} ${exe_sym_sh_addr} ${exe_sym_sh_offset} ${exe_sym_st_value}
