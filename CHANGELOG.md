...
#### Deprecated
#### Documentation
- cxl: add xcena.rst, montage-tech.rst, marvell.rst;
- mm: add c-ram.rst;
...
- add anaconda.mk, `lm_sensors.mk`, unicode/runes.mk, mbw.mk, sysbench.mk, numa.mk, flatpak.mk, asciidoc.mk, asciidoctor.mk, net/ping.mk, php.mk, libguestfs/{libguestfs.mk,virt-v2v.mk,guestfs-tools.mk};
...
#### Changed
- procfs: rename directory from `process` to `self`;
...
#### Template
- kernel.mk: add `kver_mod_x()` functions;
...
- add bits/targets.mk `add_target_objects()`, `add_target_depends()`, `add_library_depends()` and `add_library_objects()`;
...
- targets/cuda.mk: remove `target-nvcc-liba-y`, use `target-liba-y` instead;
...
#### Deprecated
#### Doc
- move all history.md to history.rst;
...
#### Security
#### Docs
- add cncf.rst, prometheus.rst, interrupt/gic.rst, interrupt/pic.rst;
#### Tools
...
#### Changed
- move algorithm/rbtree/samples to libs;
...
#### Changed
- top Makefile: remove `gitconfig` target;
- gprof: move to gnu/binutils/ dir;
...
- add memcached;
#### Template
- git.mk: remove `git_archive()`, `git_config()`, `git_clean()` functions;
...
- luca,cuda: support compile simple cmake;
