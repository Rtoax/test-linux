...
logger_infor_timer()
{
	local unit=logger.service

	# inst_simple "$moddir/logger.timer" /usr/lib/systemd/system/logger.timer
	# # /etc/systemd/system/multi-user.target.wants/logger.timer → /usr/lib/systemd/system/logger.timer
	# inst_simple "$moddir/logger.timer" /etc/systemd/system/multi-user.target.wants/logger.timer
	inst_simple "$moddir/$unit" "$systemdsystemunitdir/$unit"
	mkdir -p "$initdir/$systemdsystemunitdir/ignition-fetch.service.requires"
	ln_r "../$unit" "$systemdsystemunitdir/ignition-fetch.service.requires/$unit"

	inst_simple "$moddir/loginfo.sh" /usr/bin/loginfo.sh
}
...
install() {
	...
	#
	# ref:
	# fedora-coreos-config/overlay.d/05core/usr/lib/dracut/modules.d/10coreos-sysctl/module-setup.sh
	mkdir -p "$initdir/etc/sysctl.d"
	echo "kernel.printk_devkmsg = on" > "$initdir/etc/sysctl.d/10-dont-ratelimit-kmsg.conf"

	# see 00-journal-log-forwarding.conf
	# ref: fedora-coreos-config/overlay.d/05core/usr/lib/dracut/modules.d/99journal-conf
