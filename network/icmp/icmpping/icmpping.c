	...
	if ((icmp->icmp_type == ICMP_ECHOREPLY) && (icmp->icmp_id = ntohs(ICMP_MAGIC_ID))) {
		...
		if (ping->log)
			ping->log("%d byte from %s: icmp_req=%u ttl=%d rtt=%.3f ms\n\r",
				len, inet_ntoa(ping->from_addr.sin_addr),
...
static icmp_ping_t* icmp_ping_init(const char *ipv4, int timeout_sec,
				   int ntry, int (*log)(const char *fmt, ...))
{
	      ...
	      int (*log)(const char *fmt, ...), long int *latency)
{
	...
	if (latency)
		*latency = (long int)(usec_diff / ping->ntry);
