...
static int map_fd;
static int nr_cpus;
static long _text_addr = 0;
static bool kern_space = true;
static bool user_space = true;

static void usage(void)
{
	printf("USAGE: sampleip [-F freq] [-u] [-k] [duration]\n");
	printf("       -F freq    # sample frequency (Hertz), default 99\n");
	printf("       -k         # sampling kernel space only\n");
	printf("       -u         # sampling user space only\n");
...
static void print_ip_map(int fd)
{
	...
	for (i = 0; i < max; i++) {
		if (counts[i].ip > _text_addr && kern_space) {
			...
			printf("0x%-17llx %-32s %u\n", counts[i].ip, sym->name,
			       counts[i].count);
		} else if (counts[i].ip <= _text_addr && user_space) {
			printf("0x%-17llx %-32s %u\n", counts[i].ip, "(user)",
			       counts[i].count);
		}
	}
...
int main(int argc, char **argv)
{
	...
	/* process arguments */
	while ((opt = getopt(argc, argv, "F:ukh")) != -1) {
		switch (opt) {
		case 'F':
			freq = atoi(optarg);
			break;
