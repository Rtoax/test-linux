...
int submit_read_request(char *file_path, struct io_uring *ring)
{
	...
	/*
	 * For each block of the file we need to read, we allocate an iovec
	 * struct which is indexed into the iovecs array. This array is passed
	 * in as part of the submission. If you don't understand this, then you
	 * need to look up how the readv() and writev() system calls work.
	 */
