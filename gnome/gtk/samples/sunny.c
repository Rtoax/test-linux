...
static void open(GApplication *application, GFile **files, gint n_files,
		 const gchar *hint)
{
	gint i;
	for (i = 0; i < n_files; i++)
...
static void show_about(GSimpleAction *action, GVariant *parameter,
		       gpointer user_data)
{
...
static void quit_app(GSimpleAction *action, GVariant *parameter,
		     gpointer user_data)
{
	GList *list, *next;
	GtkWindow *win;
...
static void new_activated(GSimpleAction *action, GVariant *parameter,
			  gpointer user_data)
{
