...
struct mytype *my_search(struct rb_root *root, int num)
{
	...
	while (node) {
		struct mytype *data =
			container_of(node, struct mytype, my_node);
		if (num < data->num) {
			node = node->rb_left;
...
int my_insert(struct rb_root *root, struct mytype *data)
{
	...
	while (*tmp) {
		struct mytype *this =
			container_of(*tmp, struct mytype, my_node);
		parent = *tmp;
...
void demo_rbtree_original(void)
{
	...
	int i, ret, num,
		nums[] = { 2,	6,  9,	 8,   3,    4,	 7,	1,    12,
			   1,	5,  6,	 9,   31,   41,	 3,	41,   34,
			   1,	34, 13,	 4,   13,   4,	 13,	41,   3,
			   41,	34, 83,	 6,   4,    87,	 65,	45,   85,
			   21,	36, 64,	 74,  75,   12,	 3,	123,  1,
			   234, 1,  234, 213, 42,   34,	 234,	13,   41,
			   34,	13, 24,	 123, 4,    34,	 5,	345,  34,
			   5,	67, 45,	 67,  5467, 467, 46523, 45245 };
	struct mytype *tmp;
