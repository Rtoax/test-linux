...
pkgs_base+=( evtest )               # Input device event monitor and query tool
pkgs_base+=( expect )
pkgs_base+=( flatpak )
pkgs_base+=( gdb cgdb )
pkgs_base+=( git )
...
pkgs_desktop+=( ttyplot )
pkgs_desktop+=( wayland-utils )
pkgs_desktop+=( weston )
...
elif [[ $(is_debian_like) ]] && [[ ${have_3rd_party} ]]; then
	wget http://www.linuxidc.com/files/repo/google-chrome.list \
		-P /etc/apt/sources.list.d/

	if [[ $(is_os ubuntu:25.04) ]]; then
		wget -q -O - https://dl.google.com/linux/linux_signing_key.pub | \
			gpg --dearmor -o /usr/share/keyrings/linux_signing_key.pub
		# Then, add deb [signed-by=/usr/share/keyrings/linux_signing_key.pub arch=amd64]
	else
		wget -q -O - https://dl.google.com/linux/linux_signing_key.pub | \
			apt-key add -
	fi
...
dnf_add_packages()
{
	...
	pkgs_devel+=( json-c-devel )
	pkgs_devel+=( kmod-devel )
...
apt_add_packages()
{
	...
	pkgs_base+=( capstone-tool )
	pkgs_base+=( clang-format )
	pkgs_base+=( cloc )
	pkgs_base+=( dwarfdump )
	pkgs_base+=( fortune-mod )
	pkgs_base+=( git-subrepo )
	pkgs_base+=( gnupg2 )
	...
	pkgs_compiler+=( libatomic1 libatomic1-i386-cross )
