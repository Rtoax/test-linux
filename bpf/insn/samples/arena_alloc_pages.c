...
BPF_INSN_SAMPLE_FUNC_PROTO(arena_alloc_pages)
{
	int bpf_arena_alloc_pages_id;
	int bpf_arena_free_pages_id;

	bpf_arena_alloc_pages_id = btf_has_kfunc("bpf_arena_alloc_pages");
	bpf_arena_free_pages_id = btf_has_kfunc("bpf_arena_free_pages");
	...
	*insn++ = BPF_CALL_KFUNC(0, bpf_arena_alloc_pages_id);
