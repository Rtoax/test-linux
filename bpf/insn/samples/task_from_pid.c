 ...
 *
 * Refs:
 * - https://docs.ebpf.io/linux/kfuncs/bpf_task_from_pid/
 */
#include <stdio.h>
...
BPF_INSN_SAMPLE_FUNC_PROTO(task_from_pid)
{
	...
	bpf_task_from_pid_id = btf_has_kfunc(NULL, "bpf_task_from_pid", true);
	bpf_task_release_id = btf_has_kfunc(NULL, "bpf_task_release", true);

	if (bpf_task_from_pid_id <= 0 || bpf_task_release_id <= 0) {
		fprintf(stderr, "ERROR: not found bpf_task_from_pid kfunc.\n");
		*cnt = 0;
		return NULL;
	}
	...
	*insn++ = BPF_CALL_KFUNC(0, bpf_task_from_pid_id);
