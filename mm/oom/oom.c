...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'e':
		if (!strcmp(arg, "glibc"))
			env.ops_type = OP_GLIBC;
		else if (!strcmp(arg, "mmap-anon"))
			env.ops_type = OP_MMAP_ANON;
		...
		break;
	case 's':
		env.mem_size = str2size(arg);
		break;
#ifdef HAVE_LIBCGROUP
	case 'M':
		env.limit_in_bytes = str2size(arg);
		break;
#endif
	case 'r':
		env.rate_limit = str2size(arg);
		break;
	case 'a':
		env.oom_adj = atoi(arg);
		break;
	case 'c':
		env.oom_score_adj = atoi(arg);
		break;
	case 'p':
		env.flag_popen = true;
		break;
...
void try_oom(struct oom_operations *ops)
{
	...
	while (keep_going) {
		...
		if (env.verbose) {
			BACK_PRINTF("allocated %ld B (%ld MiB, %ld GiB), %.2lf MiB/s,"
				    " oom_score %d, adj %d, score_adj %d",
...
int main(int argc, char *argv[])
{
	...
	if (env.oom_adj) {
		...
		err = set_oom_adj(getpid(), env.oom_adj);
		if (err != 0)
			return err;
	}

	if (env.oom_score_adj) {
		printf("set oom_score_adj to %d\n", env.oom_score_adj);
		err = set_oom_score_adj(getpid(), env.oom_score_adj);
		if (err != 0)
			return err;
	}
	...
	if (env.verbose) {
		...
		printf("rate_limit %ld\n", env.rate_limit);
	}

	if (env.mem_size) {
		ops->total_size = env.mem_size;
		hold_mem(ops);
	} else {
		try_oom(ops);
	}
