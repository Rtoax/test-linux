...
ssize_t my_read(struct file *file, char *buf, size_t count, loff_t *pos)
{
	...
	/* determine which physical page to temporarily map and how far into
	 * that page to begin reading from */
	page_number = *pos / PAGE_SIZE;
	page_indent = *pos % PAGE_SIZE;

	/* map the designated physical page into kernel space.
	 * If kerel vesion is 2.6.32 or later, please use pfn_to_page() to get
	 * page, and include asm-generic/memory_model.h */
